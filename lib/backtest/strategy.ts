...
// This is the Track-2 "strategy skill": Euphoria's market-psychology thesis
// expressed as PURE, reproducible rules over OHLCV — no LLM, no randomness, so it
// can be backtested and unit-tested. It mirrors the live agents' philosophy:
// Scout (momentum), Crowd (FOMO), Reverse (bubble risk).
//
// Thesis: the crowd buys euphoria and sells fear. So we ride healthy momentum but
// REFUSE to chase euphoric blow-off tops, and we step aside when over-extension
// (bubble) is high. Long-only, signal-driven.

import { clamp } from "@/lib/utils";
import type { Candle, StrategyFeatures, BacktestSignal } from "@/types/backtest";
...
export function computeFeatures(window: Candle[]): StrategyFeatures {
  ...
  // 7-day run-up (or as much history as we have).
 ...
 * crowd psychology — with hysteresis (enter > 55, exit < 42, hold between) to
 * avoid whipsaw:
 *  • SELL  — risk-off: either a euphoric blow-off (high FOMO + high bubble) OR a
 *            broken trend / crowd fear (momentum collapses). Move to cash.
